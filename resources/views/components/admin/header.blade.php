...
<header class="sticky top-0 z-40 border-b border-slate-200/80 bg-white/95 backdrop-blur supports-[backdrop-filter]:bg-white/75">
    <div class="flex items-center justify-between px-6 py-4">
        ...
        <div class="flex items-center gap-3" data-dropdown>
            <button type="button" data-dropdown-toggle="admin-profile-menu" class="flex items-center gap-2.5 rounded-full border border-slate-200/80 bg-white px-3 py-1.5 text-xs font-semibold text-slate-600 shadow-sm transition hover:-translate-y-0.5 hover:border-[#0b3019]/40 hover:text-[#0b3019]">
                <span class="flex h-8 w-8 items-center justify-center overflow-hidden rounded-full bg-[#0b3019]/10 text-sm font-semibold text-[#0b3019]">
                    @if ($profileUrl)
                ...
                <span class="uppercase tracking-[0.2em] text-[11px] text-[#0b3019]">{{ Str::upper($firstName) }}</span>
            ...
            <div id="admin-profile-menu" data-dropdown-menu class="invisible absolute right-0 top-full mt-3 w-56 translate-y-1 rounded-2xl border border-slate-200/80 bg-white py-2 text-sm text-slate-600 opacity-0 shadow-xl transition duration-200 ease-out">
                <div class="flex items-center gap-3 px-4 pb-3">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-slate-400">{{ $user->email ?? 'admin@acses.edu' }}</p>
                    </div>
                </div>
