...
<x-layouts.admin :title="$title">
	...
	<div x-data="{ loading: true }" x-init="setTimeout(() => { loading = false }, 600)" class="relative">
	    <div x-show="loading" x-transition.opacity.duration.200ms class="pointer-events-none absolute inset-0 z-10 flex justify-center bg-slate-50/80 backdrop-blur-sm" role="status" aria-live="polite">
	        <div class="mx-auto w-full max-w-6xl space-y-8 px-5 py-10 sm:px-6 lg:px-8">
	            <header class="flex flex-col gap-4 rounded-3xl border border-[#0b3019]/15 bg-white/90 p-6 shadow-lg shadow-[#0b3019]/10">
	                <div class="space-y-2">
	                    <div class="skeleton inline-flex h-7 w-48 items-center rounded-full bg-[#0b3019]/10"></div>
	            ...
	            <section class="space-y-6 rounded-3xl border border-[#0b3019]/10 bg-white p-6 shadow-lg shadow-[#0b3019]/10">
	                ...
	                <div class="grid gap-3 md:grid-cols-4">
	                    ...
	                    <div class="space-y-2">
	                        ...
	                        <div class="skeleton h-11 w-full rounded-2xl bg-[#0b3019]/10"></div>
	                    </div>
	                </div>
	    ...
	    <div x-show="!loading" x-transition.opacity.duration.200ms x-cloak class="mx-auto w-full max-w-6xl space-y-10 px-5 py-10 sm:px-6 lg:px-8">
	        <header class="flex flex-col gap-4 rounded-3xl border border-[#0b3019]/15 bg-white/85 p-6 shadow-lg shadow-[#0b3019]/5">
	            <div class="space-y-2">
	                ...
	                <h1 class="text-2xl font-semibold text-[#0b3019] sm:text-3xl">Review student suggestions</h1>
	        ...
	        <section class="space-y-6 rounded-3xl border border-[#0b3019]/10 bg-white p-6 shadow-lg shadow-[#0b3019]/10">
	                ...
	                <article class="rounded-2xl border border-slate-200/80 bg-slate-50 px-5 py-4 shadow-sm">
	                    <div class="flex items-center justify-between">
	                        <div>
	                            ...
	                            <p class="mt-2 text-3xl font-semibold text-[#0b3019]">{{ number_format($metrics['total']) }}</p>
	                        </div>
	                        <span class="flex h-10 w-10 items-center justify-center rounded-2xl bg-[#0b3019]/10 text-[#0b3019]">
	                ...
	                <article class="rounded-2xl border border-slate-200/80 bg-slate-50 px-5 py-4 shadow-sm">
	                    <div class="flex items-center justify-between">
	                        <div>
	                            ...
	                            <p class="mt-2 text-2xl font-semibold text-[#0b3019]">{{ number_format($metrics['pending']) }}</p>
	                        </div>
	                        <span class="flex h-9 w-9 items-center justify-center rounded-xl bg-white text-[#0b3019]">
                ...
                <div class="flex w-full flex-col gap-2 md:w-64">
                    ...
                    <div class="relative">
                        ...
                        <input id="filter_search" type="search" name="search" value="{{ $filters['search'] ?? '' }}" placeholder="Subject, message, student" class="h-11 w-full rounded-2xl border border-slate-200 bg-white pl-11 pr-4 text-sm text-slate-700 shadow-sm transition focus:border-[#0b3019] focus:outline-none focus:ring-2 focus:ring-[#0b3019]/30" />
                    </div>
                </div>
                ...
                <div class="flex w-full flex-col gap-2 md:w-52">
                    ...
                    <div class="relative">
                        <select id="filter_category" name="category" class="h-11 w-full appearance-none rounded-2xl border border-slate-200 bg-white pl-4 pr-10 text-sm text-slate-700 shadow-sm transition focus:border-[#0b3019] focus:outline-none focus:ring-2 focus:ring-[#0b3019]/30">
            ...
            <div class="flex flex-col gap-4 rounded-2xl border border-slate-200/70 bg-white/60 p-4 text-sm text-slate-600 md:flex-row md:items-center md:justify-between">
                ...
                <form method="GET" class="flex items-center gap-2">
                    ...
                    <select id="suggestions_per_page" name="per_page" class="h-9 rounded-xl border border-slate-200 bg-white px-3 text-xs text-slate-700 shadow-sm focus:border-[#0b3019] focus:ring-[#0b3019]" onchange="this.form.submit()">
                    ...
                    <div class="flex flex-col gap-4 rounded-xl border border-slate-200/80 bg-slate-50/60 px-4 py-3 md:flex-row md:items-center md:justify-between">
                        ...
                        <div class="flex flex-col gap-3 md:flex-row md:items-center md:gap-4" x-show="selectedIds.length" x-cloak x-transition.opacity>
                            <div class="flex flex-col gap-1">
                                ...
                                <select id="suggestion_bulk_status" x-model="statusValue" class="w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm text-slate-700 focus:border-[#0b3019] focus:ring-[#0b3019]/40 md:min-w-[12rem]">
                            ...
                            <button type="button" class="inline-flex items-center gap-2 rounded-xl bg-[#0b3019] px-4 py-2 text-sm font-semibold text-white transition hover:-translate-y-0.5 hover:bg-[#0b3019]/90 disabled:opacity-60" @click="submit('update_status')" :disabled="!canApplyStatus">
                                        ...
                                        <tr>
                                            ...
                                            <td class="px-5 py-3">
                                                <div class="flex items-center justify-end">
                                                    <a href="{{ route('admin.suggestions.show', $suggestion) }}" class="inline-flex items-center gap-1 rounded-xl border border-slate-200 px-3 py-1.5 text-[12px] font-semibold text-slate-600 transition hover:border-[#0b3019]/40 hover:text-[#0b3019]">
                                ...
                                @forelse ($suggestions as $suggestion)
                                    <article class="rounded-2xl border border-slate-200/70 bg-white p-5 shadow-sm">
                                        ...
                                        <footer class="mt-4 flex items-center justify-between">
                                            <input type="checkbox" name="ids[]" value="{{ $suggestion->id }}" class="h-4 w-4 rounded border-slate-300 text-[#0b3019] focus:ring-[#0b3019]" @change="toggle({{ $suggestion->id }}, $event.target.checked)" :checked="selectedIds.includes({{ $suggestion->id }})">
                                            <a href="{{ route('admin.suggestions.show', $suggestion) }}" class="inline-flex items-center gap-1 rounded-xl border border-slate-200 px-3 py-1.5 text-xs font-semibold text-slate-600 transition hover:border-[#0b3019]/40 hover:text-[#0b3019]">
                                                <i class="ri-eye-line text-sm"></i>
                                                View
                                            </a>
                                        </footer>
