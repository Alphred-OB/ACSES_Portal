...
<x-layouts.admin :title="$title">
    <div class="mx-auto w-full max-w-4xl space-y-8 px-5 py-10 sm:px-6 lg:px-8">
        ...
        <header class="space-y-3 rounded-3xl border border-[#0b3019]/15 bg-white/85 p-6 shadow-lg shadow-[#0b3019]/10">
            <div class="flex flex-wrap items-start justify-between gap-4">
                <div class="space-y-2">
                    ...
                    <h1 class="text-2xl font-semibold text-[#0b3019]">{{ $suggestion->subject }}</h1>
                </div>
                @php
        ...
        <section class="space-y-4 rounded-3xl border border-slate-200/80 bg-white p-6 shadow-lg shadow-[#0b3019]/10">
        ...
        <section class="rounded-3xl border border-slate-200/80 bg-white p-6 shadow-lg shadow-[#0b3019]/10">
            ...
            <form method="POST" action="{{ route('admin.suggestions.update', $suggestion) }}" class="mt-4 flex flex-col gap-3 md:flex-row md:items-center">
                ...
                <label class="flex w-full flex-col gap-2 text-sm text-slate-600 md:w-64">
                    ...
                    <select name="status" class="h-11 w-full rounded-2xl border border-slate-200 bg-white px-4 text-sm text-slate-700 shadow-sm focus:border-[#0b3019] focus:outline-none focus:ring-2 focus:ring-[#0b3019]/30">
                        @foreach ($statuses as $value => $label)
                            <option value="{{ $value }}" @selected($suggestion->status === $value)>{{ $label }}</option>
                        @endforeach
                    </select>
                </label>
                <button type="submit" class="inline-flex h-11 items-center justify-center gap-2 rounded-2xl bg-[#0b3019] px-6 text-sm font-semibold uppercase tracking-[0.2em] text-white shadow-lg shadow-[#0b3019]/20 transition hover:-translate-y-0.5 hover:bg-[#0b3019]/90">
        ...
        @if ($suggestion->attachment_path)
            <section class="rounded-3xl border border-slate-200/80 bg-white p-6 shadow-lg shadow-[#0b3019]/10">
                ...
                <a href="{{ Storage::disk('public')->url($suggestion->attachment_path) }}" target="_blank" rel="noopener" class="mt-4 inline-flex items-center gap-2 rounded-2xl border border-[#0b3019]/30 px-4 py-2 text-sm font-semibold text-[#0b3019] transition hover:-translate-y-0.5 hover:border-[#0b3019]">
