...
<x-layouts.admin :title="$title">
    ...
    <div x-data="{ loading: true, showSummary: true }" x-init="setTimeout(() => { loading = false }, 600)" class="mx-auto w-full max-w-6xl space-y-10 px-5 py-10 sm:px-6 lg:px-8">
        <div x-show="loading" x-transition.opacity.duration.200ms class="space-y-8" role="status" aria-live="polite">
            <header class="flex flex-col gap-4 rounded-3xl border border-[#0b3019]/15 bg-white/85 p-6 shadow-lg shadow-[#0b3019]/10">
                <div class="space-y-2">
                    <div class="skeleton inline-flex h-7 w-44 items-center rounded-full bg-[#0b3019]/10"></div>
            ...
            <section class="space-y-6 rounded-3xl border border-[#0b3019]/10 bg-white p-6 shadow-lg shadow-[#0b3019]/10">
                <div class="grid gap-4 md:grid-cols-3">
                    <article class="rounded-2xl border border-[#0b3019]/20 bg-[#0b3019] px-6 py-5 text-white shadow-lg shadow-[#0b3019]/20">
                        <div class="space-y-3">
                    ...
                    @for ($i = 0; $i < 2; $i++)
                        <article class="rounded-2xl border border-slate-200/70 bg-slate-50/80 px-6 py-5 shadow-sm shadow-[#0b3019]/5">
                            <div class="space-y-3">
        ...
        <div x-show="!loading" x-transition.opacity.duration.200ms x-cloak class="space-y-10">
            <header class="flex flex-col gap-4 rounded-3xl border border-[#0b3019]/15 bg-white/85 p-6 shadow-lg shadow-[#0b3019]/10 md:flex-row md:items-center md:justify-between">
                <div class="space-y-2">
                    ...
                    <h1 class="text-2xl font-semibold text-[#0b3019] md:text-3xl">Manage academic year dues</h1>
                    <p class="text-sm text-slate-600">Review issued dues, monitor collections, and configure amounts for each programme cohort.</p>
                </div>
                <div class="flex flex-col items-stretch gap-3 sm:flex-row sm:items-center">
                    <a href="{{ route('admin.dues.export', request()->query()) }}" class="inline-flex items-center justify-center gap-2 rounded-2xl border border-[#0b3019]/20 bg-white px-4 py-2 text-xs font-semibold uppercase tracking-[0.2em] text-[#0b3019] shadow-sm transition hover:-translate-y-0.5 hover:bg-white/90">
                        <i class="ri-download-2-line text-base" aria-hidden="true"></i>
                        Export Excel
                    </a>
                    <a href="{{ route('admin.dues.statistics', request()->query()) }}" class="inline-flex items-center justify-center gap-2 rounded-2xl border border-[#0b3019]/20 bg-white px-4 py-2 text-xs font-semibold uppercase tracking-[0.2em] text-[#0b3019] shadow-sm transition hover:-translate-y-0.5 hover:bg-white/90">
                        <i class="ri-pie-chart-2-line text-base" aria-hidden="true"></i>
                        View analytics
                    </a>
                    <button type="button" @click="showSummary = !showSummary" class="inline-flex items-center justify-center gap-2 rounded-2xl border border-[#0b3019]/20 bg-white px-4 py-2 text-xs font-semibold uppercase tracking-[0.2em] text-[#0b3019] shadow-sm transition hover:-translate-y-0.5 hover:bg-white/90">
                        <i class="ri-layout-3-line text-base" aria-hidden="true"></i>
            ...
            <section class="space-y-6 rounded-3xl border border-[#0b3019]/10 bg-white p-6 shadow-lg shadow-[#0b3019]/10">
                <div class="grid gap-4 md:grid-cols-4" x-show="showSummary" x-transition.opacity.duration.150ms>
                    <article class="rounded-2xl border border-[#0b3019]/20 bg-[#0b3019] px-6 py-5 text-white shadow-lg shadow-[#0b3019]/20">
                        <p class="text-xs font-semibold uppercase tracking-[0.35em] text-white/75">Total invoices</p>
                        <p class="mt-3 text-4xl font-semibold">{{ number_format($totals['count'] ?? 0) }}</p>
                        <p class="mt-2 text-xs uppercase tracking-[0.25em] text-white/60">Across selected filters</p>
                    </article>

                    <article class="rounded-2xl border border-slate-200/70 bg-slate-50/80 px-6 py-5 shadow-sm shadow-[#0b3019]/5">
                        <p class="text-xs font-semibold uppercase tracking-[0.35em] text-slate-500">Outstanding balance</p>
                        <p class="mt-3 text-3xl font-semibold text-[#0b3019]">GHS {{ number_format((float) ($totals['outstanding'] ?? 0), 2) }}</p>
                        <p class="mt-2 text-xs text-slate-500">Sum of owing and pending verification dues.</p>
                    </article>

                    <article class="rounded-2xl border border-slate-200/70 bg-white px-6 py-5 shadow-sm shadow-[#0b3019]/5">
                        <p class="text-xs font-semibold uppercase tracking-[0.35em] text-slate-500">Total billed</p>
                        <p class="mt-3 text-3xl font-semibold text-slate-900">GHS {{ number_format((float) ($totals['total'] ?? 0), 2) }}</p>
                        <p class="mt-2 text-xs text-slate-500">Includes paid, pending verification, and outstanding dues.</p>
                    </article>

                    <article class="rounded-2xl border border-slate-200/70 bg-emerald-50/80 px-6 py-5 shadow-sm shadow-[#0b3019]/5">
                        <p class="text-xs font-semibold uppercase tracking-[0.35em] text-emerald-700">Total paid</p>
                        <p class="mt-3 text-3xl font-semibold text-[#0b3019]">GHS {{ number_format((float) ($totals['paid'] ?? 0), 2) }}</p>
                        <p class="mt-2 text-xs text-emerald-800/90">Confirmed payments across selected filters.</p>
                    </article>
                </div>
                ...
                <form method="GET" class="grid gap-4 rounded-2xl border border-slate-200/70 bg-slate-50/70 p-4 md:grid-cols-6">
                    ...
                    <div class="flex flex-col gap-2">
                        ...
                        <div class="relative">
                            <select name="class" class="h-11 w-full appearance-none rounded-2xl border border-slate-200 bg-white pl-4 pr-10 text-sm text-slate-700 shadow-sm transition focus:border-[#0b3019] focus:outline-none focus:ring-2 focus:ring-[#0b3019]/30">
                    ...
                    <div class="flex flex-col gap-2">
                        ...
                        <div class="relative">
                            <select name="year" class="h-11 w-full appearance-none rounded-2xl border border-slate-200 bg-white pl-4 pr-10 text-sm text-slate-700 shadow-sm transition focus:border-[#0b3019] focus:outline-none focus:ring-2 focus:ring-[#0b3019]/30">
                                <option value="">All years</option>
                    ...
                    <div class="md:col-span-2 md:col-start-5 md:row-start-2 flex items-end justify-end gap-3 sm:items-center sm:justify-end">
                        ...
                        <button type="submit" class="inline-flex h-11 w-full items-center justify-center gap-2 rounded-2xl bg-[#0b3019] px-6 text-sm font-semibold uppercase tracking-[0.2em] text-white shadow-lg shadow-[#0b3019]/20 transition hover:-translate-y-0.5 hover:bg-[#0b3019]/90 sm:w-auto">
                        ...
                        @forelse ($dues as $due)
                            <article class="rounded-2xl border border-slate-200/70 bg-white p-5 shadow-sm">
                                <header class="space-y-1">
                                    <h2 class="text-base font-semibold text-slate-900">{{ $due->description }}</h2>
                                ...
                                <dl class="mt-3 space-y-1 text-xs text-slate-500">
                                    ...
                                    <div class="flex justify-between">
                                        <dt>Due date</dt>
                                        <dd>{{ optional($due->due_date)->format('M j, Y') ?? $due->due_date }}</dd>
                                    </div>
                                    <div class="flex justify-between">
                                        <dt>Status</dt>
                                        <dd>{{ $statusLabels[$due->payment_status] ?? ucfirst(str_replace('_', ' ', $due->payment_status)) }}</dd>
                                    </div>
                                </dl>
                            </article>
                        @empty
