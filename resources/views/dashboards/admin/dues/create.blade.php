...
<x-layouts.admin :title="$title">
    <div class="mx-auto w-full max-w-5xl space-y-10 px-5 py-10 sm:px-6 lg:px-8">
        <header class="space-y-3 rounded-3xl border border-[#0b3019]/15 bg-white p-6 shadow-lg shadow-[#0b3019]/10">
            ...
            <h1 class="text-3xl font-semibold text-[#0b3019]">Configure dues for the academic year</h1>
        ...
        <form method="POST" action="{{ route('admin.dues.store') }}" class="space-y-8">
            ...
            <section class="space-y-6 rounded-3xl border border-[#0b3019]/10 bg-white p-6 shadow-lg shadow-[#0b3019]/10">
                ...
                <div class="grid gap-5 md:grid-cols-2">
                    <label class="flex flex-col gap-2">
                        ...
                        <input type="text" name="description" value="{{ old('description') }}" required maxlength="255" placeholder="Departmental dues 2025" class="h-11 rounded-2xl border border-slate-200 bg-white px-4 text-sm text-slate-700 shadow-sm transition focus:border-[#0b3019] focus:outline-none focus:ring-2 focus:ring-[#0b3019]/30">
                        @error('description')
                    ...
                    <label class="flex flex-col gap-2">
                        ...
                        <div class="relative">
                            <input type="text" name="academic_year" value="{{ old('academic_year') }}" required placeholder="2025/2026" pattern="^\d{4}/\d{4}$" class="h-11 w-full rounded-2xl border border-slate-200 bg-white px-4 text-sm text-slate-700 shadow-sm transition focus:border-[#0b3019] focus:outline-none focus:ring-2 focus:ring-[#0b3019]/30">
                ...
                <div class="grid gap-5 md:grid-cols-3">
                    <label class="flex flex-col gap-2 md:col-span-2">
                        ...
                        <input type="date" name="due_date" value="{{ old('due_date') }}" required class="h-11 rounded-2xl border border-slate-200 bg-white px-4 text-sm text-slate-700 shadow-sm transition focus:border-[#0b3019] focus:outline-none focus:ring-2 focus:ring-[#0b3019]/30">
                        @error('due_date')
                            <span class="text-xs text-rose-600">{{ $message }}</span>
                        @enderror
                    </label>
                    <label class="flex flex-col gap-2">
                        ...
                        <input type="number" step="0.01" min="0" name="base_amount" value="{{ old('base_amount', '0.00') }}" required class="h-11 rounded-2xl border border-slate-200 bg-white px-4 text-sm text-slate-700 shadow-sm transition focus:border-[#0b3019] focus:outline-none focus:ring-2 focus:ring-[#0b3019]/30">
            ...
            <section class="space-y-6 rounded-3xl border border-[#0b3019]/10 bg-white p-6 shadow-lg shadow-[#0b3019]/10">
                <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                        <h2 class="text-lg font-semibold text-[#0b3019]">Class &amp; year matrix</h2>
