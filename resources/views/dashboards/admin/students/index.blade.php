...
<x-layouts.admin :title="$title">
    ...
    <div x-data="{ loading: true }" x-init="setTimeout(() => { loading = false }, 600)" class="mx-auto w-full max-w-6xl px-5 py-10 sm:px-6 lg:px-8">
        <div x-show="loading" x-transition.opacity.duration.200ms class="space-y-8" role="status" aria-live="polite">
            <header class="flex flex-col gap-4 rounded-3xl border border-[#0b3019]/15 bg-white/80 p-6 shadow-lg shadow-[#0b3019]/5">
                <div class="space-y-2">
                    <div class="skeleton inline-flex h-7 w-40 items-center rounded-full bg-[#0b3019]/10"></div>
            ...
            <section class="space-y-6 rounded-3xl border border-[#0b3019]/10 bg-white p-6 shadow-lg shadow-[#0b3019]/10">
                <div class="grid gap-4 lg:grid-cols-4">
                    ...
                    <div class="lg:col-span-3 grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
                        @for ($i = 0; $i < 3; $i++)
                            <article class="rounded-2xl border border-slate-200/80 bg-white px-5 py-4 shadow-sm shadow-[#0b3019]/5">
                                <div class="space-y-3">
                ...
                <div class="mt-6 space-y-4 rounded-2xl border border-slate-200/80 bg-slate-50/70 p-4">
                    <div class="grid gap-3 md:grid-cols-3">
                        ...
                        <div class="flex items-end justify-start md:justify-end">
                            <div class="skeleton h-11 w-36 rounded-2xl bg-[#0b3019]/10"></div>
                        </div>
                    </div>
                </div>
        ...
        <header class="flex flex-col gap-4 rounded-3xl border border-[#0b3019]/15 bg-white/80 p-6 text-center shadow-lg shadow-[#0b3019]/5 sm:text-left md:flex-row md:items-center md:justify-between">
            <div class="space-y-2">
                ...
                <h1 class="text-2xl font-semibold text-[#0b3019] md:text-3xl">Manage student accounts</h1>
            ...
            <div class="flex w-full flex-col gap-2 sm:w-auto sm:flex-row sm:flex-wrap sm:justify-end">
                <a href="{{ route('admin.students.create') }}" class="inline-flex w-full items-center justify-center gap-2 rounded-2xl bg-[#0b3019] px-5 py-2.5 text-sm font-semibold text-white shadow-lg shadow-[#0b3019]/20 transition hover:-translate-y-0.5 hover:shadow-xl sm:w-auto" aria-label="Create student account">
                ...
                <a href="{{ route('admin.students.export', request()->query()) }}" class="inline-flex w-full items-center justify-center gap-2 rounded-2xl border border-[#0b3019]/30 bg-white px-5 py-2.5 text-sm font-semibold text-[#0b3019] shadow-sm transition hover:-translate-y-0.5 hover:shadow-lg sm:w-auto" aria-label="Export students">
        ...
        <section class="space-y-6 rounded-3xl border border-[#0b3019]/10 bg-white p-6 shadow-lg shadow-[#0b3019]/10">
                        ...
                        @forelse ($stats['class_breakdown'] as $classStat)
                            <article class="rounded-2xl border border-slate-200/80 bg-white px-5 py-4 shadow-sm shadow-[#0b3019]/5">
                                <div class="flex items-start justify-between gap-3">
                                    <div class="space-y-1">
                                        <div class="flex items-center gap-2 text-sm font-semibold text-[#0b3019]">
                ...
                <div class="space-y-4">
                    ...
                    <div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                        @forelse (($stats['graduated_class_breakdown'] ?? []) as $gradStat)
                            <article class="rounded-2xl border border-slate-200 bg-white px-4 py-3 shadow-sm shadow-[#0b3019]/5">
                                <div class="flex items-center justify-between gap-2">
                                    <div class="space-y-1">
                                        <p class="text-xs font-semibold uppercase tracking-[0.22em] text-slate-500">{{ $gradStat['name'] }}</p>
                                        <p class="text-xl font-semibold text-[#0b3019]">{{ number_format($gradStat['total'] ?? 0) }}</p>
                                        <p class="text-[11px] font-medium uppercase tracking-[0.2em] text-slate-400">graduates</p>
                                    </div>
                                    <span class="flex h-9 w-9 items-center justify-center rounded-xl bg-[#0b3019]/5 text-[#0b3019]">
            ...
            <form method="GET" class="space-y-3 rounded-2xl border border-slate-200/80 bg-slate-50/70 p-4 md:flex md:flex-wrap md:items-end md:gap-3 md:space-y-0">
                ...
                <div class="flex w-full flex-col gap-2 md:w-64">
                    ...
                    <div class="relative">
                        ...
                        <input id="filter_search" type="search" name="search" value="{{ $filters['search'] ?? '' }}" placeholder="Name, username, email" class="h-11 w-full rounded-2xl border border-slate-200 bg-white pl-11 pr-4 text-sm text-slate-700 shadow-sm transition focus:border-[#0b3019] focus:outline-none focus:ring-2 focus:ring-[#0b3019]/30" />
                    </div>
                </div>
                ...
                <div class="flex items-end md:ml-auto md:w-auto">
                    <button type="submit" class="inline-flex h-11 min-w-[140px] items-center justify-center gap-2 rounded-2xl bg-[#0b3019] px-5 text-sm font-semibold uppercase tracking-[0.2em] text-white shadow-lg shadow-[#0b3019]/20 transition hover:-translate-y-0.5 hover:bg-[#0b3019]/90">
            ...
            <div class="flex flex-col gap-4 rounded-2xl border border-slate-200/70 bg-white/60 p-4 text-sm text-slate-600 md:flex-row md:items-center md:justify-between">
                ...
                <form method="GET" class="flex items-center gap-2">
                    ...
                    <select id="students_per_page" name="per_page" class="h-9 rounded-xl border border-slate-200 bg-white px-3 text-xs text-slate-700 shadow-sm focus:border-[#0b3019] focus:ring-[#0b3019]" onchange="this.form.submit()">
                                ...
                                <tr>
                                    ...
                                    <td class="px-5 py-3">
                                        <div class="flex items-center justify-end gap-1.5">
                                            <a href="{{ route('admin.students.show', $student) }}" class="inline-flex items-center gap-1 rounded-xl border border-slate-200 px-3 py-1.5 text-[12px] font-semibold text-slate-600 transition hover:border-[#0b3019]/40 hover:text-[#0b3019]">
                                                <i class="ri-eye-line text-sm"></i>
                                                View
                                            </a>
                                            <a href="{{ route('admin.students.edit', $student) }}" class="inline-flex items-center gap-1 rounded-xl border border-slate-200 px-3 py-1.5 text-[12px] font-semibold text-slate-600 transition hover:border-[#0b3019]/40 hover:text-[#0b3019]">
                        ...
                        @forelse ($students as $student)
                            <article class="rounded-2xl border border-slate-200/70 bg-white p-5 shadow-sm">
                                ...
                                <footer class="mt-4 flex items-center justify-end gap-2">
                                    <a href="{{ route('admin.students.show', $student) }}" class="inline-flex items-center gap-1 rounded-xl border border-slate-200 px-3 py-1.5 text-xs font-semibold text-slate-600 transition hover:border-[#0b3019]/40 hover:text-[#0b3019]">
                                        <i class="ri-eye-line text-sm"></i>
                                        View
                                    </a>
                                    <a href="{{ route('admin.students.edit', $student) }}" class="inline-flex items-center gap-1 rounded-xl border border-slate-200 px-3 py-1.5 text-xs font-semibold text-slate-600 transition hover:border-[#0b3019]/40 hover:text-[#0b3019]">
                                        <i class="ri-pencil-line text-sm"></i>
                                        Edit
                                    </a>
