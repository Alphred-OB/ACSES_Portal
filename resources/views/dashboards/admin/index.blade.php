...
<x-layouts.admin :title="$title">
    ...
    <div x-data="{ loading: true }" x-init="setTimeout(() => { loading = false }, 600)" class="mx-auto w-full max-w-6xl px-5 py-10 sm:px-6 lg:px-8">
        <div x-show="loading" x-transition.opacity.duration.200ms class="space-y-10" role="status" aria-live="polite">
            <section class="overflow-hidden rounded-[28px] border border-[#0b3019]/15 bg-[#0b3019] p-10 shadow-[0_24px_60px_-30px_rgba(11,48,25,0.45)]">
                <div class="space-y-4">
            ...
            <section class="grid gap-6 lg:grid-cols-3">
                <article class="rounded-3xl border border-[#0b3019]/15 bg-white p-6 shadow-lg shadow-[#0b3019]/10 lg:col-span-2">
                    <div class="space-y-4">
                ...
                <aside class="space-y-6">
                    <article class="rounded-3xl border border-[#0b3019]/15 bg-white p-6 shadow-lg shadow-[#0b3019]/10">
                        <div class="space-y-3">
        ...
        <div x-show="!loading" x-transition.opacity.duration.200ms x-cloak class="space-y-10">
            <section class="animate-fade-slide overflow-hidden rounded-[28px] border border-[#0b3019]/15 bg-gradient-to-br from-[#0b3019] via-[#0f3f22] to-[#0b3019] p-10 text-white shadow-[0_24px_60px_-30px_rgba(11,48,25,0.4)]">
                <div class="flex flex-col gap-6 md:flex-row md:items-center md:justify-between">
                    <div class="space-y-3">
                        <p class="text-xs font-semibold uppercase tracking-[0.35em] text-emerald-100/80">ACSES Admin Console</p>
            ...
            <section class="grid gap-6 lg:grid-cols-4">
                @foreach ($overviewCards as $card)
                    <article class="animate-fade-slide rounded-3xl border border-[#0b3019]/10 bg-white p-6 shadow-lg shadow-[#0b3019]/10">
                        <div class="flex items-start justify-between">
                            <div class="space-y-2">
                                <p class="text-xs font-semibold uppercase tracking-[0.25em] text-slate-400">{{ $card['label'] }}</p>
                                <p class="text-2xl font-semibold text-[#0b3019]">{{ $card['value'] }}</p>
                        ...
                        @if (!empty($card['link']))
                            <a href="{{ $card['link'] }}" class="mt-6 inline-flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.2em] text-[#0b3019]/70 transition hover:text-[#0b3019]">
            ...
            <aside class="space-y-6 lg:col-span-3">
                <article class="animate-fade-slide rounded-3xl border border-[#0b3019]/15 bg-white p-6 shadow-lg shadow-[#0b3019]/10">
                    <header class="flex items-center justify-between">
                        <h2 class="text-lg font-semibold text-[#0b3019]">Upcoming events</h2>
                        <a href="{{ route('admin.events.index') }}" class="text-xs font-semibold uppercase tracking-[0.2em] text-[#0b3019]/60 hover:text-[#0b3019]">View all</a>
                    </header>
                    <ul class="mt-4 space-y-4">
                        @forelse ($upcomingEvents as $event)
                            <li class="rounded-2xl border border-slate-200/70 bg-white/70 p-4">
                                ...
                                <p class="mt-1 text-xs uppercase tracking-[0.25em] text-[#0b3019]/70">{{ $event['schedule'] ?? 'TBA' }}</p>
                                <div class="mt-2 flex flex-wrap items-center gap-2 text-xs text-slate-500">
                                    <span class="inline-flex items-center rounded-full bg-[#0b3019]/10 px-3 py-1 font-semibold uppercase tracking-[0.2em] text-[#0b3019]">{{ $event['category'] }}</span>
                ...
                <article class="rounded-3xl border border-[#0b3019]/15 bg-white p-6 shadow-lg shadow-[#0b3019]/10">
                    <header class="flex items-center justify-between">
                        <h2 class="text-lg font-semibold text-[#0b3019]">Latest suggestions</h2>
                        <a href="{{ route('admin.resources.index') }}" class="text-xs font-semibold uppercase tracking-[0.2em] text-[#0b3019]/60 hover:text-[#0b3019]">View resources</a>
                    </header>
                        ...
                        @forelse ($recentSuggestions as $suggestion)
                            <li class="rounded-2xl border border-slate-200/70 bg-white/70 p-4">
                                ...
                                <div class="mt-2 flex flex-wrap items-center gap-2 text-xs uppercase tracking-[0.2em] text-slate-400">
                                    ...
                                    <span class="rounded-full bg-[#0b3019]/10 px-3 py-1 font-semibold text-[#0b3019]">{{ $suggestion['status'] }}</span>
                                </div>
