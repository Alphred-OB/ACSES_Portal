    ...
    <div class="mx-auto w-full max-w-4xl space-y-6 px-5 py-10 sm:px-6 lg:px-8">
        ...
        <header class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between animate-fade-slide">
            <div class="space-y-1">
                ...
                <h1 class="text-2xl font-bold tracking-tight text-slate-900">Add a timeline checkpoint</h1>
                <p class="text-sm text-slate-500">Define the milestone window, add optional CTAs, and publish it instantly for students.</p>
            </div>
        ...
        <section class="rounded-2xl border border-slate-100 bg-white p-5 shadow-sm animate-fade-slide animate-fade-slide-delay-200">
            <form method="POST" action="{{ route('admin.timeline.store') }}" class="space-y-5">
                @csrf
