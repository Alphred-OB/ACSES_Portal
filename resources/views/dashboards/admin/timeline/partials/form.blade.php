...
<div class="space-y-6">
    <div class="grid gap-5 md:grid-cols-2">
        <div class="space-y-2">
            ...
            <input id="title" name="title" type="text" value="{{ old('title', $entry->title) }}" required maxlength="150" class="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-700 shadow-sm focus:border-[#0b3019] focus:ring-[#0b3019]" placeholder="e.g. Semester registration opens">
            @error('title')
        ...
        <div class="space-y-2">
            ...
            <input id="academic_year" name="academic_year" type="text" value="{{ old('academic_year', $entry->academic_year) }}" maxlength="15" class="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-700 shadow-sm focus:border-[#0b3019] focus:ring-[#0b3019]" placeholder="e.g. 2024/2025">
            @error('academic_year')
                <p class="text-xs text-rose-600">{{ $message }}</p>
            @enderror
        </div>
    ...
    <div class="space-y-2">
        ...
        <input id="starts_at" name="starts_at" type="date" value="{{ $startValue }}" required class="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-700 shadow-sm focus:border-[#0b3019] focus:ring-[#0b3019]">
        @error('starts_at')
            <p class="text-xs text-rose-600">{{ $message }}</p>
        @enderror
    </div>
    ...
    <div class="flex items-center justify-between rounded-2xl border border-slate-200/80 bg-slate-50 px-4 py-3">
        <div>
            <p class="text-sm font-semibold text-[#0b3019]">Publish immediately</p>
