		...
		<div x-show="!loading" x-transition.opacity.duration.200ms x-cloak class="space-y-10">
			<header class="flex flex-col gap-4 rounded-3xl border border-[#0b3019]/15 bg-white/80 p-6 text-center shadow-lg shadow-[#0b3019]/5 sm:text-left md:flex-row md:items-center md:justify-between">
				<div class="space-y-2">
					...
					<h1 class="text-2xl font-semibold text-[#0b3019] md:text-3xl">Orchestrate key academic milestones</h1>
				...
				<div class="flex flex-wrap items-center justify-center gap-3 md:justify-end">
					<a href="{{ route('admin.timeline.create') }}" class="inline-flex items-center gap-2 rounded-2xl bg-[#0b3019] px-5 py-2.5 text-sm font-semibold text-white shadow-lg shadow-[#0b3019]/20 transition hover:-translate-y-0.5 hover:shadow-xl" aria-label="Create timeline entry">
			...
			<section class="space-y-6 rounded-3xl border border-[#0b3019]/10 bg-white p-6 shadow-lg shadow-[#0b3019]/10">
				<div class="flex flex-col gap-4 text-center sm:flex-row sm:items-center sm:justify-between sm:text-left">
					<div>
						<h2 class="text-lg font-semibold text-[#0b3019]">Published milestones</h2>
					...
					<form method="GET" class="flex flex-col items-center gap-2 sm:flex-row" x-data>
						...
						<select id="per_page" name="per_page" class="w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm text-slate-700 shadow-sm focus:border-[#0b3019] focus:ring-[#0b3019] sm:w-auto" x-on:change="$el.form.submit()">
								...
								@forelse ($entries as $entry)
									<tr class="transition hover:bg-slate-50/70">
										<td class="px-6 py-4 align-top">
											<div class="space-y-1">
												<p class="flex items-center gap-2 text-sm font-semibold text-[#0b3019]">
										...
										<td class="px-6 py-4 align-top text-sm text-slate-500">
											<p><i class="ri-calendar-line mr-1 text-[#0b3019]" aria-hidden="true"></i>{{ optional($entry->starts_at)->format('M j, Y') }}</p>
										</td>
										...
										<td class="px-6 py-4 text-right text-sm">
											<div class="flex items-center justify-end gap-3">
												<a href="{{ route('admin.timeline.edit', $entry) }}" class="inline-flex items-center gap-2 rounded-xl border border-[#0b3019]/20 px-3 py-1.5 text-xs font-semibold text-[#0b3019] transition hover:border-[#0b3019] hover:text-[#0b3019]"><i class="ri-edit-line text-sm" aria-hidden="true"></i>Edit</a>
									...
									<tr>
										<td colspan="5" class="px-6 py-10 text-center text-sm text-slate-500">
											<div class="mx-auto flex w-full max-w-md flex-col items-center gap-4">
												<span class="inline-flex h-14 w-14 items-center justify-center rounded-full bg-[#0b3019]/10 text-[#0b3019]">
												...
												<a href="{{ route('admin.timeline.create') }}" class="inline-flex items-center gap-2 rounded-2xl bg-[#0b3019] px-4 py-2 text-sm font-semibold text-white shadow-lg shadow-[#0b3019]/20 transition hover:-translate-y-0.5 hover:shadow-xl">
													<i class="ri-add-line text-base" aria-hidden="true"></i>
													Create milestone
												</a>
											</div>
						...
						@foreach ($entries as $entry)
							<article class="space-y-4 p-5">
								<header class="space-y-1">
									<p class="flex items-center gap-2 text-sm font-semibold text-[#0b3019]"><i class="ri-flag-2-line text-base" aria-hidden="true"></i>{{ $entry->title }}</p>
								</header>
								<dl class="space-y-1 text-xs text-slate-500">
									<div class="flex items-start gap-2">
										<dt class="font-semibold text-[#0b3019]">Date</dt>
										<dd>{{ optional($entry->starts_at)->format('M j, Y') }}</dd>
									</div>
									<div class="flex items-start gap-2">
										<dt class="font-semibold text-[#0b3019]">Year</dt>
										<dd>{{ $entry->academic_year ?? '—' }}</dd>
									</div>
									<div class="flex items-start gap-2">
										<dt class="font-semibold text-[#0b3019]">Status</dt>
										<dd>
											@if ($entry->is_published)
