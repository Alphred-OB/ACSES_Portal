    ...
    <div class="mx-auto w-full max-w-4xl space-y-6 px-5 py-10 sm:px-6 lg:px-8">
        ...
        <header class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between animate-fade-slide">
            <div class="space-y-1">
                ...
                <h1 class="text-2xl font-bold tracking-tight text-slate-900">Update resource</h1>
                <p class="text-sm text-slate-500">Refine targeting, descriptions, or replace files to keep materials relevant.</p>
            </div>
        ...
        <form method="POST" action="{{ route('admin.resources.update', $resource) }}" enctype="multipart/form-data" class="space-y-5" x-data="resourceForm({
                ...
                <div class="grid gap-4 md:grid-cols-2">
                    <div class="space-y-2">
                        ...
                        <div class="grid gap-2 sm:grid-cols-3">
                            @foreach ($resourceTypes as $type)
                                <label class="inline-flex items-center gap-1.5 rounded-lg border px-3 py-1.5 text-xs font-semibold transition cursor-pointer" :class="resourceType === '{{ $type }}' ? 'border-[#0b3019] bg-[#0b3019]/10 text-[#0b3019]' : 'border-slate-200 text-slate-600 hover:border-[#0b3019]/40 hover:text-[#0b3019]'">
                    ...
                    <div class="md:col-span-2" x-show="resourceType === 'file'" x-cloak>
                        ...
                        @if ($resource->file_path)
                            <p class="mt-1.5 text-xs text-slate-500">Current file: <span class="font-medium text-[#0b3019]">{{ basename($resource->file_path) }}</span></p>
                        @endif
                    </div>
                </div>
            </section>
