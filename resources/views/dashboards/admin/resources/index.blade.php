		...
		<div x-show="loading" x-transition.opacity.duration.200ms class="space-y-8" role="status" aria-live="polite">
			<header class="flex flex-col gap-4 rounded-3xl border border-[#0b3019]/15 bg-white/80 p-6 text-center shadow-lg shadow-[#0b3019]/5 sm:text-left md:flex-row md:items-center md:justify-between">
				...
				<div class="flex flex-wrap items-center justify-center gap-3 md:justify-end">
					<div class="skeleton h-10 w-40 rounded-2xl bg-[#0b3019]/15"></div>
				</div>
			</header>
		...
		<div x-show="!loading" x-transition.opacity.duration.200ms x-cloak class="space-y-10">
			<header class="flex flex-col gap-4 rounded-3xl border border-[#0b3019]/15 bg-white/80 p-6 text-center shadow-lg shadow-[#0b3019]/5 sm:text-left md:flex-row md:items-center md:justify-between">
				<div class="space-y-2">
					...
					<h1 class="text-2xl font-semibold text-[#0b3019] md:text-3xl">Curate resources for every cohort</h1>
				...
				<div class="flex flex-wrap items-center justify-center gap-3 md:justify-end">
					<a href="{{ route('admin.resources.create') }}" class="inline-flex items-center gap-2 rounded-2xl bg-[#0b3019] px-5 py-2.5 text-sm font-semibold text-white shadow-lg shadow-[#0b3019]/20 transition hover:-translate-y-0.5 hover:shadow-xl" aria-label="Create new academic resource">
			...
			<section class="space-y-6 rounded-3xl border border-[#0b3019]/10 bg-white p-6 shadow-lg shadow-[#0b3019]/10">
				<div class="flex flex-col gap-4 text-center sm:flex-row sm:items-center sm:justify-between sm:text-left">
					<div>
						<h2 class="text-lg font-semibold text-[#0b3019]">Resource library</h2>
									...
									<tr>
											...
											<div class="space-y-1">
												...
												<div class="flex flex-wrap gap-2 pt-2">
													@if ($resource->category)
														<span class="inline-flex items-center gap-1 rounded-full bg-[#0b3019]/10 px-2.5 py-1 text-[11px] font-semibold uppercase tracking-[0.2em] text-[#0b3019]/80">
															<i class="fa-solid {{ $resource->icon ?? 'fa-book-open-reader' }} text-[#0b3019]"></i>
										...
										<td class="px-6 py-4">
											<div class="flex flex-wrap items-center gap-2">
												...
												@foreach ($classes as $class)
													<span class="inline-flex items-center gap-1 rounded-full bg-[#0b3019]/10 px-3 py-1 text-xs font-semibold text-[#0b3019]">
										...
										<td class="px-6 py-4">
											<div class="flex items-center justify-end gap-2">
												<a href="{{ route('admin.resources.edit', $resource) }}" class="inline-flex items-center gap-2 rounded-xl border border-slate-200 px-3 py-1.5 text-xs font-semibold text-slate-600 transition hover:border-[#0b3019]/40 hover:text-[#0b3019]">
						...
						@if ($resources->count())
							...
							<article class="flex flex-col gap-4 rounded-2xl border border-slate-200/80 bg-white p-5 shadow-sm">
								...
								<div class="flex flex-wrap gap-2 text-xs text-slate-600">
									...
									@endif
									@foreach ($classes as $class)
										<span class="inline-flex items-center gap-1 rounded-full bg-[#0b3019]/10 px-3 py-1 font-semibold text-[#0b3019]">
											<i class="ri-stack-line text-sm"></i>
											{{ $class }}
										</span>
									@endforeach
