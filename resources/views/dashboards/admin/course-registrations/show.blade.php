...
<x-layouts.admin :title="$title">
    ...
    <div class="mx-auto w-full max-w-5xl space-y-8 px-5 py-10 sm:px-6 lg:px-8">
        <header class="flex flex-col gap-4 rounded-3xl border border-[#0b3019]/15 bg-white/80 p-6 shadow-lg shadow-[#0b3019]/5 md:flex-row md:items-center md:justify-between">
            <div class="space-y-2">
                <a href="{{ route('admin.course-registrations.index') }}" class="inline-flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.3em] text-[#0b3019]/70 transition hover:text-[#0b3019]">
                    <i class="ri-arrow-go-back-line"></i>
                    Back to registrations
                </a>
                <h1 class="text-2xl font-semibold text-[#0b3019] md:text-3xl">{{ $student?->fullname ?? $student?->username ?? 'Unknown student' }}</h1>
        ...
        <div class="grid gap-8 lg:grid-cols-3">
            <section class="space-y-6 rounded-3xl border border-slate-200/80 bg-white p-6 shadow-lg shadow-[#0b3019]/5 lg:col-span-2">
                <header class="space-y-2 border-b border-slate-200 pb-4">
                    <h2 class="text-lg font-semibold text-[#0b3019]">Uploaded documents</h2>
                            ...
                            @foreach ($documents as $doc)
                                <li class="flex flex-col gap-3 rounded-2xl border border-slate-200 px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
                                    ...
                                    <div class="flex gap-2">
                                        <a href="{{ $doc['url'] }}" download class="inline-flex items-center gap-2 rounded-xl border border-slate-200 px-3 py-1.5 text-xs font-semibold text-slate-600 transition hover:border-[#0b3019]/40 hover:text-[#0b3019]">
                                            <i class="ri-download-2-line text-sm"></i>
                                            Download
                                        </a>
                                        <a href="{{ $doc['url'] }}" target="_blank" rel="noopener" class="inline-flex items-center gap-2 rounded-xl border border-slate-200 px-3 py-1.5 text-xs font-semibold text-slate-600 transition hover:border-[#0b3019]/40 hover:text-[#0b3019]">
            ...
            <section class="space-y-6 rounded-3xl border border-slate-200/80 bg-white p-6 shadow-lg shadow-[#0b3019]/5">
                <header class="space-y-2 border-b border-slate-200 pb-4">
                    <h2 class="text-lg font-semibold text-[#0b3019]">Review decision</h2>
                ...
                <form method="POST" action="{{ url('/admin/course-registrations/'.$registration->getKey()) }}" class="space-y-5">
                    ...
                    <div class="space-y-2">
                        ...
                        <select id="status" name="status" class="w-full rounded-2xl border border-slate-200 px-4 py-3 text-sm text-slate-900 shadow-sm focus:border-[#0b3019] focus:outline-none focus:ring-2 focus:ring-[#0b3019]/40">
                    ...
                    <div class="flex items-center justify-end gap-3">
                        <a href="{{ route('admin.course-registrations.index') }}" class="inline-flex items-center gap-2 rounded-full border border-slate-200 px-4 py-2 text-sm font-semibold text-slate-600 transition hover:border-[#0b3019]/40 hover:text-[#0b3019]">
                            Cancel
                        </a>
