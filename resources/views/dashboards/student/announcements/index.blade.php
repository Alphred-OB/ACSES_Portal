<x-layouts.dashboard :title="$title">
    ...
    <div x-data="{ loading: true }" x-init="setTimeout(() => { loading = false }, 600)" class="mx-auto w-full max-w-6xl space-y-10 px-4 py-12 sm:px-6 lg:px-8">
        <div x-show="loading" x-transition.opacity.duration.200ms class="space-y-8" role="status" aria-live="polite">
            <section class="hidden sm:block overflow-hidden rounded-[24px] border border-[#0b3019]/15 bg-[#0b3019] p-8 shadow-[0_20px_50px_-30px_rgba(11,48,25,0.4)] text-white">
                <div class="space-y-4">
            ...
            <div class="grid gap-4 rounded-3xl border border-slate-200/80 bg-white p-6 shadow-lg shadow-[#0b3019]/10 md:grid-cols-4">
        ...
        <div x-show="!loading" x-transition.opacity.duration.200ms x-cloak class="space-y-10">
            ...
            <form method="GET" action="{{ route('student.announcements.index') }}" class="grid gap-4 rounded-3xl border border-slate-200/80 bg-white p-6 shadow-lg shadow-[#0b3019]/10 md:grid-cols-4">
                <label class="flex flex-col gap-2">
                    <span class="text-xs font-semibold uppercase tracking-[0.25em] text-slate-400">Search</span>
                    <input type="search" name="search" value="{{ $filters['search'] }}" placeholder="Keyword, topic, department" class="rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-900 shadow-sm focus:border-[#0b3019]/60 focus:outline-none focus:ring-2 focus:ring-[#0b3019]/40">
                </label>

                <label class="flex flex-col gap-2">
                    <span class="text-xs font-semibold uppercase tracking-[0.25em] text-slate-400">Type</span>
                    <div class="relative">
                        <select name="type" class="w-full appearance-none rounded-2xl border border-slate-200 bg-white py-3 pl-4 pr-12 text-sm text-slate-900 shadow-sm focus:border-[#0b3019]/60 focus:outline-none focus:ring-2 focus:ring-[#0b3019]/40">
                ...
                <label class="flex flex-col gap-2">
                    ...
                    <div class="relative">
                        <select name="priority" class="w-full appearance-none rounded-2xl border border-slate-200 bg-white py-3 pl-4 pr-12 text-sm text-slate-900 shadow-sm focus:border-[#0b3019]/60 focus:outline-none focus:ring-2 focus:ring-[#0b3019]/40">
                            <option value="">All</option>
                ...
                <div class="flex items-end gap-2">
                    <button type="submit" class="inline-flex w-full items-center justify-center gap-2 rounded-full bg-[#0b3019] px-4 py-3 text-sm font-semibold text-white shadow transition hover:-translate-y-0.5 hover:bg-[#094018] focus:outline-none focus:ring-2 focus:ring-[#0b3019]/40">
                ...
                @else
                    <div class="space-y-5">
                        @foreach ($announcements as $announcement)
                            <article class="flex flex-col rounded-3xl border border-slate-200/80 bg-white p-5 shadow-lg shadow-[#0b3019]/5 transition hover:-translate-y-1 hover:shadow-[#0b3019]/15">
                                <div class="space-y-3">
                                ...
                                <a href="{{ route('student.announcements.show', $announcement) }}" class="mt-5 inline-flex items-center gap-2 text-sm font-semibold text-[#0b3019] transition hover:underline">
                                    Read more
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5" aria-hidden="true">
